var connection = function(options){
  ...
  self.reconnect_count = 0;  // number of times that we have reconnected
      ...
      self.redis.select(options.database, function(err){
        self.redis.info(function(err, data){
          if ( self.reconnect_count == 0 ) {
            // we only want to inform the caller about the INITIAL connection.
            // subsequent connections will occur if the redis connection is killed,
            // as happens often on Heroku
            callback(err);
          }
        });
      });
