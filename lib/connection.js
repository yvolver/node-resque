var connection = function(options){
  ...
  // number of times that we have reconnected
...
connection.prototype.connect = function(callback){
  ...
  self.redis.on('reconnecting', function() {
      ...
      self.redis.select(options.database, function(err){
        self.redis.info(function(err, data){
          if(self.reconnect_count == 0){
            // We only inform the caller about the FIRST connection
            // that we establish with redis. The redis module will
            // automatically reconnect connections that die, so this
            // 'connect' event is fired multiple times (particularly
            // on Heroku instances where connections seem to idle out)
            callback(err);
          }
        });
      });
